...
function Clock() {
  ...
  const onMouseEnter = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => { // пишут студенты // показать дату если наведена мышка
    setShow(!show)
  }
  const onMouseLeave = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => { // пишут студенты // спрятать дату если мышка не наведена
    setShow(!show)
  }

  // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты
  const stringTime = new Intl.DateTimeFormat("ru", { hour12: false, hour: "numeric", minute: "numeric", second: "numeric" }).format(date) || <br />
